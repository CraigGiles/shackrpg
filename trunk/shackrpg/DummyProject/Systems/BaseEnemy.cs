...
namespace DummyProject.Systems
{
    public abstract class BaseEnemy
    {
        ...
        /// <summary>
        /// Indicates weather or not the monster is dead
        /// </summary>
        bool bIsDead = false;
        ...
        private void HasDied(Player player)
        {
            ...
            #region Handles EXP
            int exp = 0;                                    //Holds the exp value to give to player

            int iLevelDifference = iLevel - player.Level;   //Determines the level difference
        ...
        //end DrawScrollingText(spriteBatch, gameTime, font)

        public void DrawEnemyInformation(SpriteBatch spriteBatch, GameTime gameTime, SpriteFont font)
        {
            /* When I get multiple enimes on screen, these values will be hosted
             * in the "Battle Engine" as "Enemy1Name, Enemy1Text, Enemy1TextValue" 
             * for Enemy 1, 2, 3, etc etc. Depends how many enemies I can fit on screen
             * at once. Some enemies might not show the values, or perhaps maybe I can
             * find a small font and show these values over their heads in a little chat
             * bubble or "battle information" bubble. Anyway, for now these values will do
             * since this is currently just a tech demo */

            Vector2 vNameText = new Vector2(20, 440);       
            Vector2 vHealthText = new Vector2(160, 440);
            Vector2 vHealthValue = new Vector2(220, 440);

            float fHealthPercent = ((float)Health / (float)HealthMax) * 100;    //Converts Health to a % value

            spriteBatch.DrawString(font, sName, vNameText, Color.White);        //Draws enemy name
            spriteBatch.DrawString(font, "HP:", vHealthText, Color.White);      //Draws enemy "HP:"
            spriteBatch.DrawString(font, fHealthPercent.ToString() + "%",       //Draws the health %
                vHealthValue, Color.White);
        }
