...
namespace DummyProject.Systems
{
    class Mantis : BaseEnemy
    {
        ...
        Rectangle rSpriteRect = new Rectangle(200, 220, 185, 170);
        #endregion
        ...
        public override void SetStartingStats()
        {
            ...
            HealthMax = 250;            //Max Health
            BaseStrength = 10;          //Strength not including buffs
            BaseAttackPower = 50;       //Attack Power not including buffs
        ...
        public override void Update(GameTime gameTime, Player player)
        {
            ...
            UpdateAnimations(gameTime);     //Updates animations
            ATBTimer += elapsed;            //Updates the ATB timer

            if (ATBTimer >= ATBTimerCap)    //If ATB timer is up, get action
            {
                GetBattleAction(player,gameTime);
            }//end if ATB timer is up
        }

        private void GetBattleAction(Player player, GameTime gameTime)
        {
            ATBTimer = 0.01f;                   //resets the ATBTimer for mantis

            //Get a random number
            int rNum = RandomHelper.GetRandomInt(1, 1000);

            //cast healing spell
            if ((Health / HealthMax) <= .20 && rNum >= 900)
            {
                // Cast Healing Spell
                StartAttackAnimation(gameTime);     //starts attack animation
                AddHealth(50);                      //adds 50 health to self
            }
            else
            {
                StartAttackAnimation(gameTime);     //starts attack animation 
                int Damage = Attack(gameTime);      //gets damage from "attack"
                player.TakePhysicalDamage(Damage);  //issues that damage to player
            }
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            base.DrawEnemyInformation(spriteBatch, gameTime, font);
        }
